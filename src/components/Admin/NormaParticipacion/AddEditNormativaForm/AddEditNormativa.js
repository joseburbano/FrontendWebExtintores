...
import "./AddEditNormativa.scss";

const { TextArea } = Input;

export default function AddEditNormativa(props) {
  const { setIsVisibleModal, setReloadNormativa, normativa } = props;
  ...
  function subirfoto(gh) {
    ...
    if (avatarUrl) {
      ...
      notification["success"]({
        message: "Subio Correctamente.",
      });
    }
  }

  //funcion para actualizar normativas
  const updateNormativa = () => {
    normativa.estado = true;
    const token = getAccessTokenApi();
    updateParticipacionApi(token, normativaData._id, normativaData)
      .then((response) => {
        const typeNotification = response.code === 200 ? "success" : "warning";
        notification[typeNotification]({
          message: response.message,
        ...
        setIsVisibleModal(false);
        setReloadNormativa(true);
        subirfotoUpda(normativaData._id);
        setNormativaData({});
      })
      .catch(() => {
        notification["error"]({
...
function AddEditForm(props) {
  ...
  return (
    ...
    >
      ...
      <Row gutter={24} className="dato1">
        <Col span={24}>
          ...
          >
            ...
            <Option value="Lesiones incidentes laborales">
              Lesiones, incidentes laborales
            </Option>
      ...
      >
        {normativa ? "Actualizar Normativa" : "Crear Normativa"}
      </Button>
    </Form>
  );
}
